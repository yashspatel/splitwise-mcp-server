"""
Splitwise MCP Server (Async) — Single Create Tool (Shares-based) Why this design: - LLM clients sometimes call the wrong tool (e.g. always equal split). - So we expose ONLY ONE "create expense" tool that supports: - Equal split (auto-computed) - No split (100/0) - Unequal split (explicit owed amounts) - Percent split (owed percentages) - Single payer (default) or multi-payer (optional) Important: - Splitwise Python SDK is synchronous. - We wrap SDK calls using asyncio.to_thread(...) to keep MCP tools async.
"""
...
async def splitwise_create_expense_shares(
    ...
    # Default payer (optional): if paid_share not provided, payer pays 100% paid_by: str = "me"
    paid_by: str = "me",
    ...
    if splits:
        ...
        # Compute owed shares if uses_percent:
        if uses_percent:
            pct_total = Decimal("0")
            ...
            # Fix rounding drift by adjusting the last owed_share drift = total_cost - sum(owed_list, Decimal("0"))
    ...
    # Case B: no splits -> equal split among participants
        ...
        # Fix drift on last drift = total_cost - sum(owed_list, Decimal("0"))
        drift = total_cost - sum(owed_list, Decimal("0"))
        if drift != Decimal("0"):
            owed_list[-1] = _d2(owed_list[-1] + drift)
    ...
    # Ensure all paid_share are Decimal and 2dp for e in split_entries:
    for e in split_entries:
    ...
    # Fix tiny owed drift by adjusting last owed participant owed_drift = total_cost - owed_total
